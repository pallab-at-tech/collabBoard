...
const CoumnAllSettings = ({columnId , columnName}) => {
    ...
    return (
        <div className="w-40 bg-white shadow-lg border border-gray-200 rounded-md overflow-hidden text-sm text-gray-800">
            ...
            <button onClick={()=>setRenameColumn(true)} className="w-full px-4 py-2 hover:bg-gray-100 flex items-center gap-2">
                <HiPencil /> Rename Column
            </button>

            <button onClick={()=>setOpenDeleteColumn(true)} className="w-full px-4 py-2 hover:bg-gray-100 text-red-500 flex items-center gap-2">
